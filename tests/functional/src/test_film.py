...
GOOD_ID = '00af52ec-9345-4d66-adbe-50eb917f463a'
...
async def test_search(make_get_request, query_info, expected_answer):
    ...
    response = await make_get_request(FILMS_ENDPOINT, query_params)
...
@pytest.mark.asyncio
async def test_film_by_id_good_id(make_get_request):
    response = await make_get_request(f'{FILMS_ENDPOINT}/{GOOD_ID}')
...
@pytest.mark.asyncio
async def test_film_by_id_bad_id(make_get_request):
    response = await make_get_request(f'{FILMS_ENDPOINT}/{BAD_ID}')
...
async def test_retrieve_from_cache(delete_doc, make_get_request, clear_cache):
    ...
    index = test_settings.es_index_mapping['movies']
